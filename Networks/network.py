...
class MODEL_base(nn.Module):
    def __init__(self, img_size=120,patch_size=4,embed_dim=96,num_heads=8, window_size=1,in_channel=2, out_channel=16,output_channel=1,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., patch_norm=True,depth=0,downsample=None,
...
class DCCA(nn.Module): # deeplab
    ...
    def __init__(self, dim,in_dim):
        ...
        self.eca1 = eca_layer(in_dim,3)
        self.eca2 = eca_layer(in_dim,3)
        self.eca3 = eca_layer(in_dim,3)
        self.eca4 = eca_layer(in_dim,3)
    ...
    def forward(self, x):
        ...
        conv3 = self.conv3(x)
        conv3 = self.eca3(conv3)
        conv4 = self.conv4(x)
        conv4 = self.eca4(conv4)
        conv5 = F.upsample(self.conv5(F.adaptive_avg_pool2d(x, 1)), size=x.size()[2:], mode='bilinear')
        return self.fuse(torch.cat((conv1, conv2, conv3,conv4, conv5), 1))



class MODEL_two_DCCA(MODEL_base):
    def __init__(self, img_size=120,patch_size=4,embed_dim=96,num_heads=8, window_size=1,in_channel=2, out_channel=16,output_channel=1,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., patch_norm=True,depth=2,downsample=None,
                 drop_path=0., norm_layer=nn.LayerNorm,use_checkpoint=False ):
        super(MODEL_two_DCCA, self).__init__()
        self.muiltscale = DCCA(out_channel, out_channel)
    ...
    def forward(self, input):
        ...
        mulit = self.muiltscale(de_add)
        out=self.convolutional_out (mulit)
        return out


class MODEL_three_DCCA(MODEL_base):
    def __init__(self, img_size=120,patch_size=4,embed_dim=96,num_heads=8, window_size=1,in_channel=2, out_channel=16,output_channel=1,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., patch_norm=True,depth=3,downsample=None,
                 drop_path=0., norm_layer=nn.LayerNorm,use_checkpoint=False ):
        super(MODEL_three_DCCA, self).__init__()
        self.muiltscale = DCCA(out_channel, out_channel)
...
class MODEL_two_TFM_TFM(MODEL_base):
    def __init__(self, img_size=120,patch_size=4,embed_dim=96,num_heads=8, window_size=1,in_channel=2, out_channel=16,output_channel=1,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., patch_norm=True,depth=2,downsample=None,
                 drop_path=0., norm_layer=nn.LayerNorm,use_checkpoint=False ):
        super(MODEL_two_TFM_TFM, self).__init__()
        
        patches_resolution = self.patch_embed.patches_resolution
        self.patches_resolution = patches_resolution
        self.tsfm=BasicLayer(dim= out_channel,
                                   input_resolution=(patches_resolution[0],patches_resolution[1]),
                                         depth=1,
                                         num_heads=num_heads,
                                         window_size=window_size,
    ...
    def forward(self, input):
        ...
        encode_size = (de_add.shape[2], de_add.shape[3])
        # Transformer=self.basicLayer(encode, encode_size)

        mulit = self.tsfm(de_add , encode_size)
        out=self.convolutional_out (mulit)
        return out
